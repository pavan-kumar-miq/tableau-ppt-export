  ...
  async getJobById(jobId) {
    try {
      ...
      if (error.message && error.message.includes('WRONGTYPE')) {
        logger.warn('Key exists but has wrong type', { jobId, error: error.message });
        return null;
      }
  ...
  async startWorker(processor) {
    ...
    const processLoop = async () => {
      while (this.isProcessing) {
        try {
          ...
          logger.error('Error in worker loop for getting next job from queue', error);
    ...
    for (let i = 0; i < this.concurrency; i++) {
      processLoop().catch(err => {
        logger.error('Worker loop for getting next job from queue crashed', err, { workerIndex: i });
      });
    }
  }
      ...
      for (const jobId of processingJobIds) {
        try {
          ...
          if (!job) {
            ...
            await this.redis.srem(this.processingKey, jobId);
            cleaned++;
            logger.warn('Removed orphaned job from processing set (job not found)', { jobId });
            continue;
          }
          ...
          if (job.status === 'processing' && job.startedAt) {
            ...
            if (processingTime > maxProcessingTime) {
              ...
              await this.updateJobById(jobId, {
                status: 'pending',
                error: 'Job was stuck in processing state and has been requeued'
              });
              await this.redis.rpush(this.queueKey, jobId);
              requeued++;
  ...
  async getQueueStats() {
    try {
      ...
      if (pendingJobIds.length > 0) {
        for (const jobId of pendingJobIds.slice(0, 100)) { // Limit to first 100 for performance
          try {
            const job = await this.getJobById(jobId);
            if (job) {
            ...
            logger.debug('Error getting pending job details', { jobId, error: error.message });
          }
        }
      }
      ...
      if (processingJobIds.length > 0) {
        for (const jobId of processingJobIds) {
          try {
            ...
            logger.debug('Error getting processing job details', { jobId, error: error.message });
          }
        }
      }
      
      // Calculate average processing time
      const avgProcessingTime = processingCountWithTime > 0 
      ...
      const jobsWithRetries = [...pendingJobsDetails, ...processingJobsDetails]
        .filter(job => job.attempts > 0);
      const totalRetries = jobsWithRetries.reduce((sum, job) => sum + job.attempts, 0);
      const avgRetries = jobsWithRetries.length > 0 
      ...
      return {
        ...
        timing: {
          ...
          oldestPendingJobCreatedAt: oldestPendingJob ? new Date(oldestPendingJob.createdAt).toISOString() : null,
